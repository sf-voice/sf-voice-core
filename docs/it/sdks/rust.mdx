---
title: "Rust"
description: "SDK Rust asincrono per l'API di ricerca. Costruito su reqwest e tokio."
---

`sf_voice_media` — asincrono, costruito su `reqwest`. Richiede `tokio`.

Usa questo SDK all'interno di servizi o job Tokio in cui ingestione e polling devono far parte dello stesso runtime asincrono.
...
`poll_task` accetta opzionalmente `PollOptions { interval_ms, timeout_ms }`. Valori predefiniti: intervallo 1500ms, timeout 120s.
