...
`sh.sfvoice:sf-voice-media-kotlin` — coroutine-native Kotlin client built on Ktor. All methods are `suspend` functions.
...
// build.gradle.kts
dependencies {
    implementation("sh.sfvoice:sf-voice-media-kotlin:0.1.1")
}
```
...
```kotlin
import sh.sfvoice.media.SearchBody
