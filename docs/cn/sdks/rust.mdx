---
title: "Rust"
description: "用于搜索 API 的异步 Rust SDK。基于 reqwest 和 tokio。"
---

`sf_voice_media` —— 异步，基于 `reqwest`。需要 `tokio`。

在希望摄入和轮询都成为同一异步运行时一部分的 Tokio 服务或作业中使用此 SDK。
...
`poll_task` 可选地接收 `PollOptions { interval_ms, timeout_ms }`。默认值：间隔 1500ms，超时 120s。
