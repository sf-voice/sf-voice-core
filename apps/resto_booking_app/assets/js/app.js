...
const TokenVault = {
  mounted() {
    this.pushEventTo(this.el, "vault:loaded", {tokens: readVault()})

    this.handleEvent("vault:save", ({id, token}) => {
      const v = readVault()
      v[id] = token
      writeVault(v)
    })

    this.handleEvent("vault:remove", ({id}) => {
      const v = readVault()
      delete v[id]
      writeVault(v)
    })

    this.handleEvent("vault:copy", ({text}) => {
...
const liveSocket = new LiveSocket("/live", Socket, {
  ...
  hooks: {...colocatedHooks, TokenVault},
})
