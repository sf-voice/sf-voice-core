# apps/ellie_ai/MEMORY.md

Decisions specific to `apps/ellie_ai`. Cross-cutting decisions that *also* constrain resto live here when ellie is the primary actor; resto's `MEMORY.md` carries a one-line cross-ref back.

Repo-wide conventions live in repo-root `AGENTS.md`. Apps-wide conventions (e.g. the constants module rule) live in `../MEMORY.md`. Stack rules live in `../ELIXIR_RULES.md`.

Read this file before doing any work in `apps/ellie_ai`.

---

## 2026-05-07 — Ellie owns all Telnyx; resto owns reservations only
...
- Nightly reconciliation cron (already planned) keeps `customer_summary` from drifting on the slow-changing fields.
