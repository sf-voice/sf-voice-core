defmodule EllieAi.Tools.CancelReservation do
  @moduledoc "cancel one of the caller's reservations."
  ...
  @impl true
  def name, do: "cancel_reservation"

  @impl true
  def description do
    """
    Cancel an existing reservation belonging to the current caller.
    Confirm party_size and starts_at against what the caller says
    before calling — the tool refuses if they don't match a
    reservation on file.
    """
  end
    ...
    %{
      ...
      properties: %{
        party_size: %{type: "integer", minimum: 1,
          description: "Party size on the reservation to cancel. Used to match."},
        starts_at: %{type: "string",
          description: "Reservation's start time, ISO 8601. Used to match."}
      },
  ...
  @impl true
  def execute(
        %{"party_size" => size, "starts_at" => starts_at},
  ...
  def execute(_, _),
    do: {:error, {:permanent, "missing required args: party_size + starts_at"}}
